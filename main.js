...
class VoiceSession {
     ...
     * @param {Discord.Guild} guild The guild being connected
     */
    constructor(guild, channel) {
        this.guild = guild;
        this.channel = channel;
        /** @type {Discord.VoiceConnection} Connection stream */
        this._connection = null;
        /** @type {Discord.StreamDispatcher} Audio player */
        this._player = null;
    }
    join() {
        if (this.channel.joinable) {
            try {
                this.channel.join().then((connection) => { this._connection = connection });
            } catch (err) {
                return err
            }
        } else {
            return new Error("Unable to join the channel")
        }
    }
    /**
     * @param {string} url The youtube video to play
     */
    play(url) {
        if (this._connection) {
            if (this.channel.speakable) {
                this._player = this._connection.play(youtube(url))//this line doesn't seem to be working
            } else {
                return new Error("Cannot speak in this channel")
            }
        } else {
            return new Error("A channel needs to be joined first")
        }
    }
    pause() {
        if (this._player) {
            if (!this._player.paused) {
                ...
                return new Error("Already paused")
            }
        } else {
            return new Error("There is nothing to pause")
        }
    }
    resume() {
        if (this._player) {
            if (this._player.paused) {
                ...
                return new Error("The player isn't paused")
            }
        } else {
            return new Error("There is nothing to resume")
        }
    }
    stop() {
        if (this._player) {
            ...
            return new Error("There is nothing to stop")
        }
    }
    /**
     * Leave the channel and destroy the session
     */
    leave() {
        if (this._player) this.stop()
        this.channel.leave()
        //destroy the evidence!
        let ind = activeSessions.findIndex(s => s.guild.id == this.guild.id)
        activeSessions.splice(ind)
        this = null;
    }
}
...
client.on("message", (msg) => {
    ...
    if (msg.mentions.has(client.user, { 'ignoreDirect': false, 'ignoreEveryone': true, 'ignoreRoles': true }) && msg.content.length <= 25) {//when it's mentioned
        msg.channel.send(Embeds.prototype.default("👋 Hey there! My prefix is `" + process.env.BOT_PREFIX + "` Use `" + process.env.BOT_PREFIX + "help` to see a list of commands"));
    };
    ...
    switch (command) {
        ...
        case "add":
            ...
            if (!args[0]) { msg.channel.send(Embeds.prototype.default(`Usage: ${process.env.BOT_PREFIX}add [profanityRole/link]`)); return; }
            if (args[0].toLowerCase() == "link") {
                ...
                } else { msg.channel.send(Embeds.prototype.default(`Usage: ${process.env.BOT_PREFIX}add profanityRole <role>`)); return; }
                ...
                msg.channel.send(Embeds.prototype.default(`Usage: ${process.env.BOT_PREFIX}add [profanityRole/link]`))
            }
            break;
        ...
        case "roles":
            ...
            if (links) {
                links.forEach(l => {
                    let guild = storage.guilds.find(g => g.id == l.guild);
        ...
        case "server":
            //depreciated command
            if (msg.author.id != storage.ownerID) { msg.react(emojis.cross); return; }
            if (msg.channel.type == "dm") { msg.channel.send(Embeds.prototype.error("This command can only be used in servers")); return; };
...
var queueTick = setInterval(() => {
    if (roleQueue.length >= 1) {//only run if theres someone there
        ...
        if (!fetchSettings(member.guild.id) || storage.ignoreList.includes(member.user.id)) {//ignore if there aren't any settings for this server or they chose to be ignored
            roleQueue.pop();
            return;
        }
        ...
        if (account) {//first check if they're registered
            ...
            if ((Date.now() - account.time) > storage.cacheTime) {//outdated cache - update it and run the que on this account again
                ...
                try {
                    apiFetch('account', account.key).then(r => {//copied from handlewaitresponse()
                        ...
                        r.guilds.forEach(g => {//callback for each guild the user is in
                            if (storage.guilds.find(i => i.id == g)) return;//ignores guilds it already knows about
                            else {
...
if (!storage) { log('ERR', "The bot just tried to start without a config file!"); process.exit(1); };
if (!process.env.BOT_TOKEN) { log('ERR', "Cannot login without a token!"); process.exit(1); };
client.login(process.env.BOT_TOKEN).catch(e => log('ERR', `Failed to login. It's probably a connection issue\n${e}`));
...
function handleWaitResponse(user, content) {
    ...
    function hwrHandle(err) {
        ...
        user.send(Embeds.prototype.error(`There was an error during setup. Did you provide a valid API key? Please DM <@" + ${process.env.BOT_OWNER} + "> if you'd like help`));
    };
};
...
function searchGuilds(query) {
    ...
    let result = null;
    storage.guilds.forEach(g => {//first try the names - probably the best way to do it
    ...
    if (!result) {
        storage.guilds.forEach(g => {//next try aliases
    ...
    if (!result) {
        storage.guilds.forEach(g => {//lastly, try guild tags
...
function newGuild(id, key, owner) {
    ...
    if (!storage.guilds.find(g => g.id == id)) {//ignores if its already there
...
function fetchSettings(server) {
    let sett = storage.serverSettings[server];
    if (sett) {
        return sett;
    } else {
        return null;
...
client.once("ready", () => {
    ...
    scrollInterval = setInterval(() => {//update the message
        let scroll = [//activities to scroll through
            { status: "online", activity: { name: `"${process.env.BOT_PREFIX}"`, type: "LISTENING" } },
